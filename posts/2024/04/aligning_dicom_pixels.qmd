...
description: |
    ...
    then this post as reached its goal.
date: "04/23/2024"
categories:
  - random
---
...
- the axes might not be in the same order: maybe the first pixel in one format is the last pixel of the other format.
...
1. In my first calculation of the shift between two DICOM slices, I initially naively thought that the slices would be in order and that the first file (with name `foo00000`) would actually be the first slice. *Ha*. *Ha*. *Ha*. They are in a random order instead.
